...
describe("readWorkflowTerminalProof", () => {
	...
	it("waits for runner exit after startup failure persistence", () => withFixture(({ root, asyncDir, writeChild }) => {
		const pending = { ...runnerIdentity, state: "pending" } as ProcessTerminal;
		writeChild({ state: "running", pid: process.pid, processTerminal: pending }, pending);
		persistRunnerStartupFailure({ asyncDir: path.join(root, "child-run"), runId: "child-run", runnerProcessInstanceId: "runner-1", message: "execution import failed" });
		assert.equal(readWorkflowTerminalProof(asyncDir, asyncChild, summary({ workflowState: "failed" }), 0, 2_000).state, "pending");
		writeChild({ state: "failed", pid: process.pid, error: "execution import failed", processTerminal: pending }, observedChild());
		assert.equal(readWorkflowTerminalProof(asyncDir, asyncChild, summary({ workflowState: "failed" }), 0, 2_000).state, "observed");
	}));

	it("does not let stale not-started status bypass runner evidence", () => withFixture(({ asyncDir, writeChild }) => {
		const notStarted = { ...runnerIdentity, state: "not-started" } as ProcessTerminal;
		writeChild({ state: "failed", pid: process.pid, error: "startup failed", processTerminal: notStarted });
		assert.equal(readWorkflowTerminalProof(asyncDir, asyncChild, summary(), 0, 2_000).state, "pending");
		writeChild({ state: "failed", error: "startup failed", processTerminal: notStarted }, { ...runnerIdentity, state: "pending" } as ProcessTerminal);
		assert.equal(readWorkflowTerminalProof(asyncDir, asyncChild, summary(), 0, 2_000).state, "pending");
	}));

	it("does not accept not-started evidence for a child that did not fail", () => withFixture(({ asyncDir, writeChild }) => {
		const notStarted = { ...runnerIdentity, state: "not-started" } as ProcessTerminal;
		writeChild({ state: "complete", error: "stale error", processTerminal: notStarted });
		assert.equal(readWorkflowTerminalProof(asyncDir, asyncChild, summary(), 0, 2_000).state, "pending");
	}));
