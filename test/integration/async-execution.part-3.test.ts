...
`);
	...
	it(`revives a current workflow child from persisted ${restricted ? "restricted" : "unrestricted"} parent admission authority`, { skip: !isAsyncAvailable() || !createSubagentExecutor ? "jiti or executor not available" : undefined }, async () => {
		const agents = [makeAgent("planner", { allowedAgents: ["researcher"] }), makeAgent("researcher")];
		const parentAuthority = { version: 1 as const, ...(restricted ? { allowedAgents: ["planner", "researcher"] } : {}), denyExtensions: false, sources: restricted ? ["workflow-parent"] : [] };
		const launchAuthority = restricted ? parentAuthority : undefined;
		...
		const executor = createSubagentExecutor!({
			...
			expandTilde: (value: string) => value,
			discoverAgents: () => ({ agents }),
			childRuntime: { capabilityCeiling: launchAuthority },
		});
		mockPi.onCall({ output: "Initial workflow child complete" });
		const launch = await executor.execute(
			"workflow-parent-authority-launch",
			{ workflowScript: `return await runs.run("planner", { agent: "planner", task: "Plan", acceptance: false })`, async: true, mission: false, capabilityCeiling: launchAuthority },
			new AbortController().signal, undefined, ctx,
		) as AsyncExecutionResult;
		assert.ok(!launch.isError, launch.content[0]?.text);
		assert.ok(launch.details.asyncId);
		...
		assert.deepEqual(payload.capabilityCeiling, {
			...
			allowedAgents: ["researcher"],
			denyExtensions: false,
			sources: restricted ? ["agent:planner", "workflow-parent"] : ["agent:planner"],
		});
	});
	}
