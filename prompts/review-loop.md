...
Keep the full supplied code, branch, design, or plan as the target. Do not create, select, or sequence vertical slices, substitute a representative subset for the requested work, or invoke a slicing workflow automatically. An existing user-selected slice is valid input. Separate slice planning belongs to `/skill:vertical-slice` only when requested. Bounded fixes and delegation packets are execution details, not permission to redefine scope. Track incomplete requirements explicitly; a clean review of a subset does not complete the full target.
...
Default to a maximum of 3 review rounds unless I specify a different cap. Count a review round each time fresh-context reviewers inspect the current diff after any parent or worker implementation/fix pass. Stop early when reviewers find no P0 findings, no P1 fixes worth doing now, and no approved P2 notes that should be handled in this loop. At the start of round 2 and every second round thereafter (4, 6, ...), reread the applicable global Pi `~/.pi/agent/AGENTS.md` and the target repository's nearest `AGENTS.override.md`/`AGENTS.md` from the exact repository/cwd under review. Treat this as an instruction-context refresh, not a reason to add a round; if a required file is missing or ambiguous, report it and follow the highest-authority instructions that are available.

“Implementation requested” does not automatically require a child writer. Apply the global delegation eligibility gate before the initial implementation and before every fix iteration. Context-heavy causal design and real execution remain in the parent, especially when the parent is Sol or Astra.

The parent is the default implementation and review owner. Use Luna medium for bounded mechanics and Luna high for ordinary implementation and review. Escalate directly to Sol 5.6 medium for difficult diagnosis, interacting invariants, a substantive Luna failure, conflicting evidence, or high-impact final review; do not require Luna xhigh first. Luna xhigh/max are manual-only, not automatic writer or retry steps. Astra low is reserved for stuck/looping work or repeated concrete failures after Sol medium; never select a higher Astra tier automatically.

Match review capability to the work rather than treating review as a cheap reading task. A Sol-medium or Astra-low parent reviews directly by default. In a Sol- or Astra-parent session, never launch a Luna reviewer. If independent external review materially improves confidence, explicitly launch the reviewer at Sol medium or higher; use Astra low only when the review itself meets the extreme-complexity threshold. Luna reviewers are limited to Luna-parent sessions: for a genuinely simple read-only review there, one Luna-high reviewer is allowed only with a context-complete packet containing the exact diff/ref, relevant source/contracts, acceptance criteria, user constraints, and validation command. Multiple reviewers require independent high-impact risks and explicit justification. Use Luna high for an eligible frozen child-writer slice. Luna xhigh/max and Terra are manual-only. Delegate a child writer only when every condition holds:
...
For each review round that actually needs external review, launch at most one fresh-context `reviewer` for a simple review, or justified parallel reviewers for independent high-impact risks. Explicitly set each reviewer's model and thinking according to the parent/review rule above; do not rely on a configured Luna default in a Sol- or Astra-parent session. Each runs in a distinct temporary read-only worktree/clone/snapshot containing the same exact integrated target diff. Record base/ref and focused diff identity; never share the parent checkout or another reviewer workspace. Reviewers must inspect the applicable global Pi `~/.pi/agent/AGENTS.md` and nearest target-repository instruction files, plus the current diff directly from files and commands, and receive the exact contracts, acceptance criteria, user constraints, and validation command. On rounds 2, 4, 6, and so on, explicitly include the refreshed instruction-file paths and any changed constraints in each reviewer packet. They must not rely on main conversation history or edit files.
...
Choose review angles from the actual change when external review is justified. Common angles are correctness/regressions, tests/validation, and simplicity/maintainability. Add security, performance, docs/API contracts, or user-flow validation only for independent high-impact risks. Prefer the parent review or one appropriately tiered, context-complete reviewer over reviewer fanout; do not prefer three reviewers by default.
