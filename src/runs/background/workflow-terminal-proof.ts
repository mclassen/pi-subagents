...
export function readWorkflowChildProcessEvidence(workflowAsyncDir: string, steps: AsyncStatus["steps"]): WorkflowChildProcessEvidence {
	...
	for (const step of steps) {
		...
		// Only a launch with no runner evidence can certify that nothing started.
		if (recorded.state === "not-started" && childStatus.pid === undefined && !fs.existsSync(path.join(childDir, "process-terminal.json")) && recorded.runId === step.runId && childStatus.state === "failed" && typeof childStatus.error === "string" && childStatus.error) {
